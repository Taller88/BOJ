...
* DFS/BFS
  - 벽 부수고 이동하기 
    - 문제 포인트
      3차원 배열(방문처리를 벽을 뚫었을때 뚫지 않았을때 구분해서 진행)
 ...
 * * **
  ...
  * 내리막길+DFS
    ...
    시작노드에서 끝노드까지 갈껀데 현재 높이보다 작은 높이로만 이동이 가능하다. 이때 시작노드에서 끝노드까지 갈 수 있는 경우의 수를 구하여라 
    
     * 접근 방법
       1. 이 문제에서 가장 중요한 건 한번 이 노드를 들렸다면 이 노드가 최종까지 갈 수 있는 지 없는 지 판단이 가능하다
       2. dp 배열에는 갈 수 있는 경우의 수를 저장해둔다. 
...
#### Greedy(탐욕법)(1/4)
 ...
 #### 크루스칼 알고리즘(최소 비용 신장 트리)
   ...
   - 크루스칼 알고리즘이란 최소비용으로 모든 노드를 들리는 알고리즘을 말한다. 
   - 여기서 사용한 부수적인 알고리즘으로는 Union-Find 알고리즘을 사용하였다. Union은 연결노드끼리 인덱스가 작은 애로 부모를 잡는 것이고 find는 해당 노드의 최상위 부모를 찾아 다른 노드와 비교해서 사용할때 사용한다.
 ...
 * 최소 비용 구하기 
  ...
  * 케빈베이컨 6단계
   ...
   * 빙산 
     ...
     - 굉장히 짜증났던 문제..빙산은 1년마다 맞닿아있는 얼음의 수 만큼 녹는다. 이때 섬이 두개이상 되는 연수를 찾으라.
     - 접근 방법 
       1. deepcopy를 통해 배열을 복사한다음 한 좌표에 맞닿아있는 얼음의 개수를 구한뒤 현재 값에서 빼준다.
       2. dfs 섬의 개수 찾아준다.
       3. 문제에 나와있는 섬의 개수는 2개이상이 아닌데 다 녹아버린 경우를 check
       4. while(true)를 통해 반복
